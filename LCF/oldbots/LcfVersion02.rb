...
class LcfVersion02
  ...
  def do_on_first_tick
    if time == 0
      ...
      @destination_setters[1] = SideWalkerSetter.new @battlefield_width.to_f, @battlefield_height.to_f, @clipping_offset.to_f
    end
  end
  ...
  def calculate_destination_based_on_damage
    if @@current_destination_setter != -1
      ...
      if (time != 0) && (speed.to_i == 0) && (@@current_destination_setter == -1) && (@is_master == 1) && (@@is_pair_at_destination == 1)
        @@current_destination_setter = 0
  ...
  def got_to_destination
    ...
    @last_x_location = x
    @last_y_location = y
    determine_if_at_destination
  end

  def determine_if_at_destination
    if ((x.to_f - @x_destination).abs < 2) && ((y.to_f - @y_destination).abs < 2)
      @@is_pair_at_destination = 1
    else
      @@is_pair_at_destination = 0
    end
