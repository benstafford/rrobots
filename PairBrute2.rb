...
class PairBrute2
  ...
  def partner_location
    if @logbook.partner_location
      @logbook.partner_location
    else
      center.dup
    end
  end
  ...
  def partner_too_far_away?
    (my_location.distance_to partner_location) > DESIRED_DISTANCE_FROM_PARTNER.max
  end

  def enemy_too_far_away?
    @logbook.last_known_enemy_distance && (@logbook.last_known_enemy_distance > MAX_DISTANCE_FROM_ENEMY)
  end
